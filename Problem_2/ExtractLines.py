...
def SplitLinesRecursive(theta, rho, startIdx, endIdx, params):
    ...
    if endIdx-startIdx <= alpha.MIN_POINTS_PER_SEGMENT:
        return alpha, r, np.vstack((startIdx, endIdx)).T
    s = FindSplit(theta[startIdx:endIdx+1], rho[startIdx:endIdx+1], alpha, r)
    if s == -1:
        return alpha, r, np.vstack((startIdx, endIdx)).T
    alpha1, r1, idx1 = SplitLinesRecursive(theta, rho, startIdx, startIdx+s)
    alpha2, r2, idx2 = SplitLinesRecursive(theta, rho, startIdx+s, endIdx)
...
def FindSplit(theta, rho, alpha, r, params):
    ...
    '''
    ########## Code starts here ##########
    d = np.abs(np.multiply(rho, np.cos(theta - alpha*np.ones_like(theta))) - r * np.ones_like(rho))
    splitIdx = np.argmax(d)
    dmax = d(splitIdx)
    if dmax < params.LINE_POINT_DIST_THRESHOLD:
        return -1
    seg1 = splitIdx
    seg2 = theta.size - splitIdx
    if min([seg1, seg2]) < params.MIN_POINTS_PER_SEGMENT:
        return -1
...
def FitLine(theta, rho):
    ...
    ########## Code starts here ##########
    n = theta.size()
    alpha_y = np.sum(np.multiply(np.power(rho,2), np.sin(2*theta))) \
              - 2/n * np.sum(np.dot(np.multiply(rho,np.cos(theta)).T, np.multiply(rho,np.sin(theta))))
    alpha_x = np.sum(np.multiply(np.power(rho,2), np.cos(2*theta))) \
              - 1/n * np.sum(np.multiply(np.dot(rho.T, rho), np.cos(np.tile(theta.T,(1,n)) + np.tile(theta,(n,1))) ))
    alpha = 0.5 * np.arctan2(alpha_y, alpha_x) + np.pi/2
    r = 1/n * np.dot(rho, np.cos(theta - alpha).T)
...
def MergeColinearNeigbors(theta, rho, alpha, r, pointIdx, params):
    ...
    '''
    ########## Code starts here ##########
    nlines = np.shape(pointIdx, 1)
    for i in range(nlines-2):
        li = i + 1
        alpha,r = FitLine(theta[pointIdx[li-1,1]:pointIdx[li,2]],
                          rho[pointIdx[li-1,1]:pointIdx[li,2]])
